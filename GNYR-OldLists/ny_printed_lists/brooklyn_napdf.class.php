<?php
/**
	\file brooklyn_napdf.class.php
	
	\brief This file creates and dumps a Kings Bay (Brooklyn) meeting list in PDF form.
*/
...
/**
	...
	the PDF file.
*/
class brooklyn_napdf extends printableList implements IPrintableList
{
	...
	*/
	function OutputPDF ()
	    {
		$d = date ( "Y_m_d" );
		$this->napdf_instance->Output( "Brooklyn_PrintableList_$d.pdf", "I" );
	    }
	...
	/********************************************************************
	*/
	private function DrawSubcommittees ( $left, $top, $right, $bottom )
		{
		$y = $top;
		...
		if ( $na_dom )
			{
			if ( @$na_dom->loadHTML($this->call_curl ( "http://newyorkna.org/Events_and_Meetings/brooklyn.html" )) )
				{
		...
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_BROOKLYN_LIST_SUBCOMMITTEES );
		
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		
		$this->napdf_instance->SetXY ( $cellleft, $y );

		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_SUBCOMMITTEES );
		...
		if ( is_array ( $s_array ) && count ( $s_array ) )
			...
			for ( $c = 0; $c < $count; $c++ )
				...
				$this->napdf_instance->MultiCell ( ($right - $left), $fontSize/72, $sub_table[$c]['_description'] );
	...
	*/
	...
	/********************************************************************
	*/
	private function DrawFormats ( $left, $top, $right, $bottom, $formats )
	    {
		$y = $top + 0.125;
		...
		foreach ( $formats as $format )
			{
            $fleft = ($index < $count / 2) ? $left : ($left + $right) / 2;
            $fright = ($index < $count / 2) ? ($left + $right) / 2 : $right;
            
            if ( !$fold &&  ($index++ >= ($count /2)) )
                {
                $fold = true;
		        $this->napdf_instance->SetY ( $y );
		        }
		    ...
		    $w1 = $fleft + 0.25;
            $this->napdf_instance->SetLeftMargin ( $fleft );
            $this->napdf_instance->SetX ( $fleft );
            ...
            $this->napdf_instance->MultiCell ( ($fright - $w1), 0.13, $str );
	...
	/********************************************************************
	*/
	private function DrawRearPanel ( $left, $top, $right, $bottom )
	    {
		$y = $top + 0.125;
		...
		$this->napdf_instance->SetFont ( $this->font, 'I', ($fontSize - 4) );
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_BROOKLYN_NAME_TEXT );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_NAME_TEXT );

		$cellleft = $left;
		$this->napdf_instance->SetFont ( $fontFamily, 'B', ($fontSize) );
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$st_width = $this->napdf_instance->GetStringWidth ( _PDF_BROOKLYN_LIST_NAME );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_NAME );
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_BROOKLYN_LIST_PHONE );
		$this->napdf_instance->SetXY ( $right - $stringWidth, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_PHONE );
		$y += 0.125;
		$this->napdf_instance->SetXY ( $cellleft, $y );
		...
		$this->napdf_instance->SetXY ( $left, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_SAYING_1 );
		...
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_SAYING_2 );
		$y += 0.14;			
		$this->napdf_instance->SetXY ( $x, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_SAYING_3 );
		$y += 0.14;			
		$this->napdf_instance->SetXY ( $x, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_SAYING_4 );
		$y += 0.14;			
		$this->napdf_instance->SetXY ( $x, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_SAYING_5 );
		$y += 0.14;			
		$this->napdf_instance->SetXY ( $x, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_SAYING_6 );
		$y += 0.14;			
		$this->napdf_instance->SetXY ( $x, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_SAYING_7 );
		$y += 0.14;			
		$this->napdf_instance->SetXY ( $x, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_SAYING_8 );
		$y += 0.14;			
		$this->napdf_instance->SetXY ( $x, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_SAYING_9 );
	    }
	
	/********************************************************************
	*/
	private function DrawFrontPanel ( $left, $top, $right, $bottom, $date, $formats )
	    {
		$inTitleGraphic = "../ny_printed_lists/images/ELI_Cover_Logo.png";
		$gnyrQR = "../ny_printed_lists/images/newyorkna.org.gif";
		$bkascQR = "../ny_printed_lists/images/BrooklyQR.gif";
		...
		$this->napdf_instance->SetFont ( $this->font, 'B', ($fontSize + 4) );
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_BROOKLYN_LIST_BANNER );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		
		$this->napdf_instance->SetXY ( $cellleft, $y );

		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_BANNER );
		$y += 0.18;
		
		$this->napdf_instance->SetFont ( $this->font, 'B', ($fontSize + 1) );
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_BROOKLYN_LIST_BANNER_2.' '._PDF_BROOKLYN_LIST_BANNER_3 );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_BANNER_2.' '._PDF_BROOKLYN_LIST_BANNER_3 );
		
		$this->napdf_instance->Image ( $inTitleGraphic, ($left + 1.2), 1.0, (($right - $left) - 2.2), 0, 'PNG' );
		$this->napdf_instance->Image ( $gnyrQR, ($left + 0.27), 1.2, 0.6, 0, 'GIF' );
		$this->napdf_instance->Image ( $bkascQR, ($right - 0.775), 1.2, 0.6, 0, 'GIF' );
		
		$this->napdf_instance->SetFont ( $this->font, '', ($fontSize - 2) );

		$this->napdf_instance->SetXY ( $left + 0.15, 1.85 );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_QR_B );

		$this->napdf_instance->SetXY ( $right - 0.82, 1.85 );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_QR_A );
		...
		$this->napdf_instance->SetXY ( $cellleft + 0.18, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BROOKLYN_LIST_HELPLINE_REGION );
		$y += 0.2;
		
		$this->napdf_instance->SetFont ( $fontFamily, 'B', ($fontSize + 2) );
		$url_string = _PDF_BROOKLYN_LIST_URL;
		...
		$this->DrawSubcommittees ( $left, $this->napdf_instance->GetY ( ) + 0.1, $right, $bottom );
		
		$y = $this->napdf_instance->GetY ( ) + 0.1;
		
		$this->napdf_instance->SetXY ( $left, $y );
		$this->napdf_instance->SetFont ( $fontFamily, 'B', $fontSize );
	    $this->napdf_instance->MultiCell ( ($right - $left), $fontSize/72, utf8_decode ( _PDF_BROOKLYN_LIST ), 0, "C" );
	    }
	...
	*/
	private function RearrangeFormats ( $inFormats )
		...
		uasort ( $inFormats, 'brooklyn_napdf::sort_cmp' );
