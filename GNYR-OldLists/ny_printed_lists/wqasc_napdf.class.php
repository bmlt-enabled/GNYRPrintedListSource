<?php
/**
	\file wqasc_napdf.class.php
	
	\brief This file creates and dumps a Western Queens meeting list in PDF form.
*/
...
/**
	...
	the PDF file.
*/
class wqasc_napdf extends printableList implements IPrintableList
{
	...
	*/
	function OutputPDF ()
	    {
		$d = date ( "Y_m_d" );
		$this->napdf_instance->Output( "WesternQueens_PrintableList_$d.pdf", "I" );
	    }
	...
	*/
	private function DrawListPage ( $in_column	///< If this is true, it is the left side. If false, it is the right side column.
		...
		$fontSize = $this->napdf_instance->FontSizePt + 1.5;
		...
		while ( !$this->pos['end'] && (($this->pos['y'] + $height_one_meeting + $extra_height + ($fSizeSmall * 2)) < ($y_offset - 0.1)) )
			...
			if ( $this->pos['start'] || (isset ( $meeting['weekday_tinyint'] ) && ($current_day != $meeting['weekday_tinyint']) ) )
				...
				$this->pos['y'] += 0.025;
				$this->napdf_instance->SetXY ( $cellleft, $this->pos['y'] );
	...
	*/
	private function DrawRearPanel ( $left, $top, $right, $bottom )
		...
		$this->napdf_instance->SetFont ( $fontFamily, 'B', ($fontSize) );
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$st_width = $this->napdf_instance->GetStringWidth ( _PDF_WQASC_LIST_NAME );
		$this->napdf_instance->Cell ( 0, 0, _PDF_WQASC_LIST_NAME );
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_WQASC_LIST_PHONE );
		$this->napdf_instance->SetXY ( $right - $stringWidth, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_WQASC_LIST_PHONE );
		$y += 0.125;
		$this->napdf_instance->SetXY ( $cellleft, $y );
	...
	*/
	private function DrawFrontPanel ( $left, $top, $right, $bottom, $date, $formats )
		...
		$this->napdf_instance->SetFont ( $this->font, 'B', ($fontSize + 4) );
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_WQASC_LIST_BANNER );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		
		$this->napdf_instance->SetXY ( $cellleft, $y );

		$this->napdf_instance->Cell ( 0, 0, _PDF_WQASC_LIST_BANNER );
		$y += 0.18;
		
		$this->napdf_instance->SetFont ( $this->font, 'B', ($fontSize + 1) );
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_WQASC_LIST_BANNER_2.' '._PDF_WQASC_LIST_BANNER_3 );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_WQASC_LIST_BANNER_2.' '._PDF_WQASC_LIST_BANNER_3 );
		...
		$this->napdf_instance->Cell ( 0, 0, _PDF_WQASC_LIST_HELPLINE );
		$y += 0.2;
		
		$this->napdf_instance->SetFont ( $fontFamily, 'B', ($fontSize + 2) );
		$url_string = _PDF_WQASC_LIST_URL;
		...
		$this->napdf_instance->SetFont ( $fontFamily, 'B', $fontSize );
	    $this->napdf_instance->MultiCell ( ($right - $left), $fontSize/72, utf8_decode ( _PDF_WQASC_LIST ), 0, "C" );
	    }
	...
	*/
	private function RearrangeFormats ( $inFormats )
		...
		uasort ( $inFormats, 'wqasc_napdf::sort_cmp' );
