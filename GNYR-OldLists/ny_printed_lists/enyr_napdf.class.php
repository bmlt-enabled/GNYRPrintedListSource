<?php
/**
	\file enyr_napdf.class.php
	
	\brief This file creates and dumps a ENYR meeting list in PDF form.
*/
...
/**
	...
	the PDF file.
*/
class enyr_napdf extends printableList implements IPrintableList
{
	...
	*/
	...
	{
		$this->page_x = 4.25;		///< The width, in inches, of each page
		$this->page_y = 5.25;		///< The height, in inches, of each page.
		...
		napdf::$sort_callback = 'enyr_napdf::sort_meeting_data_callback_ny';
	...
	function AssemblePDF ()
	{
		...
		if ( $this->napdf_instance instanceof napdf )
			...
			foreach ( $all_meetings as $meeting )
				{
				if ( preg_match ( '|ES|i', $meeting['formats'] ) )
					{
	...
	*/
	...
	{
		$d = date ( "Y_m_d" );
		$this->napdf_instance->Output( "ENYR_PrintableList_$d.pdf", "D" );
	}
	...
	*/
										...
										)
	{
		...
		usort ( $ret, 'enyr_napdf::sort_cmp_formats' );
...
//				$this->napdf_instance->Cell ( 0, 0, _PDF_CHANGE_NOTE );
...
//	$this->napdf_instance->Cell ( 0, 0, _PDF_CHANGE_NOTE );
	...
	*/
	...
	uasort ( $inFormats, 'enyr_napdf::sort_cmp' );
