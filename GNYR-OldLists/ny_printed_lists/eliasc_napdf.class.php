...
/**
	...
	the PDF file.
*/
class eliasc_napdf extends tabloid_napdf
{
	/********************************************************************
	...
	*/
	...
	{
		$d = date ( "Y_m_d" );
		$this->napdf_instance->Output( "ELI_PrintableList_$d.pdf", "I" );
	}
	...
	*/
	function RearrangeFormats ( $inFormats )
	{
		...
		uasort ( $inFormats, 'eliasc_napdf::sort_cmp' );
	...
	function AssemblePDF ()
	{
		...
		if ( $this->napdf_instance instanceof napdf )
			...
			if ( $meeting_data )
                ...
                $keys = array();
                $objs = array();
                foreach ( $this->napdf_instance->format_data as $format_obj )
                    {
                    array_push ( $keys, $format_obj['key_string'] );
                    $objs[$format_obj['key_string']] = $format_obj;
                    }
                
                $keys = explode ( ',', $this->RearrangeFormats ( implode ( ',', $keys ) ) );
                
                $newObj = array();
                foreach ( $keys as $key )
                    {
                    array_push ( $newObj, $objs[$key] );
                    }
                    
                $this->napdf_instance->format_data = $newObj;
				$this->DrawRearPanel ( $backpanel_x_offset, $backpanel_y_offset, $backpanel_max_x_offset, $backpanel_max_y_offset, $newObj );
                ...
                $temp_instance = clone $this->napdf_instance;
                $temp_pos = $this->pos;
                $this->font_size += 5;
				...
				do
                    ...
                    if ( $this->pos['end'] && $this->DrawSubcommittees ( ) )
                        {
                        break;
                        }
                    else
                        {
                        $this->napdf_instance = clone $temp_instance;
                        $this->font_size -= 0.1;
                        $this->pos = $temp_pos;

                        continue;
                        }
                    } while ( $this->font_size > 0.0 );
				}
			$ret = true;
			}
	...
	*/
	function DrawFormats ( $left, $top, $right, $bottom, $formats )
	{
		...
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_ELI_ASC_LIST_FORMAT_KEY );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		
		$this->napdf_instance->SetXY ( $cellleft, $y );

		$this->napdf_instance->Cell ( 0, 0, _PDF_ELI_ASC_LIST_FORMAT_KEY );
		$y += PDF_MARGIN;
	...
	*/
	function DrawSubcommittees ( )
		...
		if ( is_array ( $s_array ) && count ( $s_array ) )
            ...
            $displayString = _PDF_ELI_ASC_LIST_SUBCOMMITTEE_HEADING;
		...
		return ( $this->napdf_instance->GetY() <= $bottom );
		}
	...
	*/
	function DrawFrontPanel ( $left, $top, $right, $bottom, $date, $formats )
	{
        ...
        $displayString = _PDF_ELI_ASC_LIST_MAIN_TITLE_TOP;
		$stringWidth = $this->napdf_instance->GetStringWidth ( $displayString );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, $displayString );
		...
		$this->napdf_instance->SetFont ( $this->font, 'B', $fontSize + 1 );
        $displayString = _PDF_ELI_ASC_LIST_MAIN_TITLE_MIDDLE;
		$stringWidth = $this->napdf_instance->GetStringWidth ( $displayString );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, $displayString );
		...
		$this->napdf_instance->SetFont ( $this->font, 'B', $fontSize + 4 );
        $displayString = _PDF_ELI_ASC_LIST_MAIN_TITLE_BOTTOM;
		$stringWidth = $this->napdf_instance->GetStringWidth ( $displayString );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, $displayString );
        ...
        $displayString = _PDF_ELI_ASC_LIST_SUB_TITLE;
		$stringWidth = $this->napdf_instance->GetStringWidth ( $displayString );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, $displayString );
		...
		$this->napdf_instance->SetFont ( $this->font, 'B', $fontSize + 2 );
        $displayString = _PDF_ELI_ASC_LIST_HELPLINE;
		$stringWidth = $this->napdf_instance->GetStringWidth ( $displayString );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, $displayString );
		...
		$this->napdf_instance->SetFont ( $this->font, 'B', $fontSize + 2 );
        $displayString = _PDF_ELI_ASC_LIST_URL;
		$stringWidth = $this->napdf_instance->GetStringWidth ( $displayString );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, $displayString );
		...
		$cellleft = $left;
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$st_width = $this->napdf_instance->GetStringWidth ( _PDF_ELI_ASC_LIST_NAME );
		$this->napdf_instance->Cell ( 0, 0, _PDF_ELI_ASC_LIST_NAME );
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_ELI_ASC_LIST_PHONE );
		$this->napdf_instance->SetXY ( $right - $stringWidth, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_ELI_ASC_LIST_PHONE );
		$y += PDF_MARGIN;
		...
		$displayString = _PDF_ELI_ASC_LIST_DISCLAIMER_LINE_1;
		$stringWidth = $this->napdf_instance->GetStringWidth ( $displayString );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, $displayString );
		$y += 0.1;
		$displayString = _PDF_ELI_ASC_LIST_DISCLAIMER_LINE_2;
		$stringWidth = $this->napdf_instance->GetStringWidth ( $displayString );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, $displayString );
		$y += 0.1;
		$displayString = _PDF_ELI_ASC_LIST_DISCLAIMER_LINE_3;
		$stringWidth = $this->napdf_instance->GetStringWidth ( $displayString );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, $displayString );
