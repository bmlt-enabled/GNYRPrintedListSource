...
/**
	...
	the PDF file.
*/
class nyna_napdf extends printableList implements IPrintableList
{
	...
	*/
							...
							)
	{
		...
		$this->sort_keys = array (	'weekday_tinyint' => true,			///< First, weekday
									...
									'location_province' => true,        ///< State
									'location_city_subsection' => true, ///< Borough
									'location_sub_province' => true,    ///< County
		...
		$this->out_http_vars = array ( 'do_search' => 'yes', 'meeting_key' => 'location_province', 'meeting_key_value' => 'NY' );

		$this->lang_search = array ( 'en', 'es' );
		
		napdf::$sort_callback = 'nyna_napdf::sort_meeting_data_callback_ny';
	...
	*/
	function AssemblePDF ()
	{
		...
		if ( $this->napdf_instance instanceof napdf )
			...
			while ( !$this->pos['end'] )
				{
				$this->DrawListPage (_PDF_SPANISH);
				}
			$ret = true;
			}
	...
	*/
	...
	{
		$d = date ( "Y_m_d" );
		$this->napdf_instance->Output( "New_York_PrintableList_$d.pdf", "I" );
	}
	...
	*/
										...
										)
	{
		...
		usort ( $ret, 'nyna_napdf::sort_cmp_formats' );
	...
	*/
	...
	/********************************************************************
	*/
	private function DrawListPage ( $in_header = null )
	{
	...
	while ( !$this->pos['end'] )
        ...
        $state = nyna_napdf::clean_state ( $meeting['location_province'] );
    
        $county = strtolower ( trim ( $meeting['location_city_subsection'] ) );
        
        if ( !$county )
            {
            $county = strtolower ( trim ( $meeting['location_sub_province'] ) );
            }
    
        $county = trim ( str_ireplace ( "county", "", $county ) );
        ...
        if ( !$county )
            ...
            if ( ($county != 'Bronx') && ($county != 'Brooklyn') && ($county != 'Manhattan') && ($county != 'Queens') && ($county != 'Staten Island') )
                {
                $county .= ' County';
                }
            }
		
		if ( $this->pos['start'] || ($current_state != $state) || ($current_county != $county) || ($current_day != $meeting['weekday_tinyint']) )
			{
			$this->napdf_instance->SetFillColor ( 0 );
			$this->napdf_instance->SetTextColor ( 255 );
			if ( $this->pos['start'] )
	...
	*/
	...
	uasort ( $inFormats, 'nyna_napdf::sort_cmp' );
	...
	*/
	...
	/**
	    \brief Cleans the state, so we always have NY, PA, MA, etc.
	*/
	static function clean_state ( $inState_raw )
	{
        $state = strtoupper ( trim ( $inState_raw ) );
        ...
        if ( 'NEW HAMPSHIRE' == $state )
            {
            $state = 'NH';
            }
            
        if ( preg_match ( '|^PEN.*|', $state ) )
            {
            $state = 'PA';
            }
            
        if ( preg_match ( '|^MAS.*|', $state ) )
            {
            $state = 'MA';
            }
	...
	*/
													...
													)
	{
		...
		$weekday_a = $in_a['weekday_tinyint'];
        $weekday_a -= napdf::$week_starts;
		...
		$weekday_b = $in_b['weekday_tinyint'];
        $weekday_b -= napdf::$week_starts;
        ...
        $start_time_a = strtotime ( $in_a['start_time'] );
        $start_time_b = strtotime ( $in_b['start_time'] );
		
        $state_a = nyna_napdf::clean_state ( $in_a['location_province'] );
        if ( $state_a == 'NY' )
            {
            $state_a = 'AA';
            }
            
        $state_b = nyna_napdf::clean_state ( $in_b['location_province'] );
        if ( $state_b == 'NY' )
            {
            $state_b = 'AA';
            }
            
        $county_a = strtolower ( trim ( $in_a['location_city_subsection'] ) );

        if ( !$county_a )
            {
            $county_a = strtolower ( trim ( $in_a['location_sub_province'] ) );
            }

        $county_a = trim ( str_ireplace ( "county", "", $county_a ) );

        if ( !$county_a )
            {
            $county_a = "Unknown County";
            }
        else
            {
            $county_a = ucwords ( $county_a );
            }

        $county_b = strtolower ( trim ( $in_b['location_city_subsection'] ) );

        if ( !$county_b )
            {
            $county_b = strtolower ( trim ( $in_b['location_sub_province'] ) );
            }

        $county_b = trim ( str_ireplace ( "county", "", $county_b ) );
        ...
        if ( !$county_b )
            {
            $county_b = "Unknown County";
            }
        else
            {
            $county_b = ucwords ( $county_b );
            }
            
		if ( $ret == 0 )
		    {
		    $ret = strcmp ( $state_a, $state_b );
		    }
		
		if ( $ret == 0 )
		    {
		    $ret = strcmp ( $county_a, $county_b );
		    }
		...
		if ( $ret == 0 )
            ...
            elseif ( $start_time_b > $start_time_a )
                {
                $ret = -1;
                }
		    }
