...
/**
	...
	the PDF file.
*/
class sas_napdf extends tabloid_napdf
{
	/********************************************************************
	...
	*/
	...
	{
		$d = date ( "Y_m_d" );
		$this->napdf_instance->Output( "Suffolk_PrintableList_$d.pdf", "I" );
	}
	...
	*/
	function DrawRearPanel ( $left, $top, $right, $bottom, $formats )
	{
		...
		$this->napdf_instance->SetFont ( $fontFamily, 'I', $fontSize * 0.63 );
		$this->napdf_instance->SetXY ( $left, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST_TIME_NOTE );
		$y = $this->napdf_instance->GetY ( ) + PDF_MARGIN;
		$this->napdf_instance->SetXY ( $left, $y );
		$this->napdf_instance->MultiCell ( ($right - $left), (($fontSize - 1) / 72), _PDF_SAS_LIST_DISCLAIMER );
		...
		$this->napdf_instance->SetXY ( $left, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST_CORRECTIONS1 );
		...
		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST_CORRECTIONS2 );
	}
	...
	*/
	function DrawSubcommittees ( )
		...
		if ( $na_dom )
			{
			if ( @$na_dom->loadHTML($this->call_curl ( "https://www.longislandna.com/meetings/sas.html" )) )
				{
		...
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_SASNA_LIST_SUBCOMMITTEES );
		...
		$this->napdf_instance->SetXY ( $left, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_SASNA_LIST_SUBCOMMITTEES );
	...
	*/
	...
	{
		$inTitleGraphic = "../ny_printed_lists/images/SASNA_Cover_Logo.jpg";
		...
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_SAS_LIST );
		
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		
		$this->napdf_instance->SetXY ( $cellleft, $y );

		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST );
		$y += 0.2;

		$this->napdf_instance->SetFont ( $this->font, 'B', ($fontSize + 9) );
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_SAS_LIST_BANNER );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		
		$this->napdf_instance->SetXY ( $cellleft, $y );

		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST_BANNER );
		$y += 0.2;
		
		$this->napdf_instance->SetFont ( $this->font, 'B', $fontSize + 1 );
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_SAS_LIST_BANNER_2.' '._PDF_SAS_LIST_BANNER_3 );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST_BANNER_2.' '._PDF_SAS_LIST_BANNER_3 );
		
		$this->napdf_instance->Image ( $inTitleGraphic, ($left + 0.50), 1.05, (($right - $left) - 1), 0, 'JPG' );

		$this->napdf_instance->SetFont ( $fontFamily, 'B', ($fontSize + 5) );
		
		$y = 2.85;

		$url_string = _PDF_SAS_LIST_URL;
		...
		$this->napdf_instance->SetXY ( $left, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST_HELPLINES );
		$y += 0.2;

		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_SAS_LIST_HELPLINE_SUFFOLK );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST_HELPLINE_SUFFOLK );
		$y += 0.16;
		
		$this->napdf_instance->SetFont ( $fontFamily, '', ($fontSize + 3) );

		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_SAS_LIST_HELPLINE_NASSAU );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST_HELPLINE_NASSAU );
		$y += 0.16;
		
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_SAS_LIST_HELPLINE_REGION );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST_HELPLINE_REGION );
		$y += 0.3;
		
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_SAS_LIST_YAP_1 );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST_YAP_1 );
		$y += 0.2;
		
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_SAS_LIST_YAP_2 );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST_YAP_2 );
		$y += 0.2;
		
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_SAS_LIST_YAP_3 );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST_YAP_3 );
		$y += 0.3;
		
		$this->napdf_instance->SetLineWidth ( 0.02 );

		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_SASNA_LIST_WELCOME );
		$this->napdf_instance->SetXY ( $left, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_SASNA_LIST_WELCOME );
		$y += 0.1;
		$this->napdf_instance->Line ( $left + $stringWidth + 0.0625, $y, $right, $y );
		$y += 0.1;

		$this->napdf_instance->SetXY ( $left, $y );
		$st_width = $this->napdf_instance->GetStringWidth ( _PDF_SAS_LIST_NAME );
		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST_NAME );
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_SAS_LIST_PHONE );
		$this->napdf_instance->SetXY ( $right - $stringWidth, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_SAS_LIST_PHONE );
		$y += PDF_MARGIN;
