...
/**
	...
	the PDF file.
*/
class bkasc_napdf extends printableList implements IPrintableList
{
	...
	*/
	private function DrawSubcommittees ( $left, $top, $right, $bottom )
		...
		if ( $na_dom )
			{
			if ( @$na_dom->loadHTML($this->call_curl ( "http://newyorkna.org/Events_and_Meetings/kba.html" )) )
				{
		...
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_BKASC_LIST_SUBCOMMITTEES );
		
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		
		$this->napdf_instance->SetXY ( $cellleft, $y );

		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_SUBCOMMITTEES );
	...
	*/
	private function DrawRearPanel ( $left, $top, $right, $bottom )
		...
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_BKASC_NAME_TEXT );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_NAME_TEXT );

		$cellleft = $left;
		$this->napdf_instance->SetFont ( $fontFamily, 'B', ($fontSize) );
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$st_width = $this->napdf_instance->GetStringWidth ( _PDF_BKASC_LIST_NAME );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_NAME );
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_BKASC_LIST_PHONE );
		$this->napdf_instance->SetXY ( $right - $stringWidth, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_PHONE );
		$y += 0.125;
		$this->napdf_instance->SetXY ( $cellleft, $y );
		...
		$this->napdf_instance->SetXY ( $left, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_SAYING_1 );
		...
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_SAYING_2 );
		$y += 0.14;			
		$this->napdf_instance->SetXY ( $x, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_SAYING_3 );
		$y += 0.14;			
		$this->napdf_instance->SetXY ( $x, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_SAYING_4 );
		$y += 0.14;			
		$this->napdf_instance->SetXY ( $x, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_SAYING_5 );
		$y += 0.14;			
		$this->napdf_instance->SetXY ( $x, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_SAYING_6 );
		$y += 0.14;			
		$this->napdf_instance->SetXY ( $x, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_SAYING_7 );
		$y += 0.14;			
		$this->napdf_instance->SetXY ( $x, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_SAYING_8 );
		$y += 0.14;			
		$this->napdf_instance->SetXY ( $x, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_SAYING_9 );
	    }
	...
	*/
	private function DrawFrontPanel ( $left, $top, $right, $bottom, $date, $formats )
		...
		$bkascQR = "../ny_printed_lists/images/mobile.newyorkna.org.gif";
		...
		$this->napdf_instance->SetFont ( $this->font, 'B', ($fontSize + 4) );
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_BKASC_LIST_BANNER );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		
		$this->napdf_instance->SetXY ( $cellleft, $y );

		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_BANNER );
		$y += 0.18;
		
		$this->napdf_instance->SetFont ( $this->font, 'B', ($fontSize + 1) );
		$stringWidth = $this->napdf_instance->GetStringWidth ( _PDF_BKASC_LIST_BANNER_2.' '._PDF_BKASC_LIST_BANNER_3 );
		$cellleft = (($right + $left) / 2) - ($stringWidth / 2);
		
		$this->napdf_instance->SetXY ( $cellleft, $y );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_BANNER_2.' '._PDF_BKASC_LIST_BANNER_3 );
		...
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_QR_B );

		$this->napdf_instance->SetXY ( $right - 1.02, 1.85 );
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_QR_A );
		...
		$this->napdf_instance->Cell ( 0, 0, _PDF_BKASC_LIST_HELPLINE_REGION );
		$y += 0.2;
		
		$this->napdf_instance->SetFont ( $fontFamily, 'B', ($fontSize + 2) );
		$url_string = _PDF_BKASC_LIST_URL;
		...
		$this->napdf_instance->SetXY ( $left, $y );
		$this->napdf_instance->SetFont ( $fontFamily, 'B', $fontSize );
	    $this->napdf_instance->MultiCell ( ($right - $left), $fontSize/72, utf8_decode ( _PDF_BKASC_LIST ), 0, "C" );
	    }
