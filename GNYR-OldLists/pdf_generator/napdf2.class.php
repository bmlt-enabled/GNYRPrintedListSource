<?php
	...
	*/
	class napdf2 extends FPDF
	{
		...
		*/
		private function FetchCSV ( $inRootURI, $in_http_vars = null	///< These contain alternatives to the $_GET and/or $_POST parameters. Default is null.
						)
			{
		...
		*/
		static function MakeNAPDF (	$inRootURI, 			///< The Root URI
			...
			self::$fpdf_instance = new napdf2 ( $inRootURI, $in_x, $in_y, $in_http_vars, $in_units, $in_orientation, $in_lang_search );
			
			if ( self::$fpdf_instance instanceof napdf2 )
				{
				if ( is_array ( self::$fpdf_instance->meeting_data ) )
					{
					if ( is_array ( $in_keys ) )
						{
		...
		*/
		private function __construct (	$inRootURI, 			///< The Root Server URI.
										$in_x,					///< The width of each printed page, in $in_units
										$in_y,					///< The height of each printed page, in $in_units
										$in_http_vars,			///< The various parameters used to dictate the meeting search.
										$in_units = 'in',		/**< The measurement units.
